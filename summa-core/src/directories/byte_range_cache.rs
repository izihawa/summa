...
mod tests {
    ...
    #[derive(Debug)]
    enum Operation {
        Insert { range: Range<usize>, tag: &'static str },
        Get { range: Range<usize>, tag: &'static str },
    }
    ...
    fn op_strategy() -> impl Strategy<Value = Operation> {
        prop_oneof![
            (tag_strategy(), range_strategy()).prop_map(|(tag, range)| Operation::Insert { range, tag }),
            (tag_strategy(), range_strategy()).prop_map(|(tag, range)| Operation::Get { range, tag }),
        ]
    }
                ...
                match op {
                    Operation::Insert {
                        ...
                        let expected_item_count: usize = state.values()
                            ...
                            .sum();
                        assert_eq!(cache.inner.read().num_items, expected_item_count as u64);

                        let expected_byte_count = state.values()
                            .flatten()
                            .filter(|stored| **stored)
                            .count();
                        assert_eq!(cache.inner.read().num_bytes, expected_byte_count as u64);
                    }
                    Operation::Get {
                        range,
                        tag,
