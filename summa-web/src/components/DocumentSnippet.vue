...
.container
  ...
  .clearfix.small
    .text-muted.mt-2.float-end
      | {{ format_bytes(filesize) }} | {{ extension }}
hr
</template>
  ...
  computed: {
    ...
    snippet() {
      ...
      if (description.length === 0) {
        description = this.document.description.substring(0, 200);
        if (this.document.description.length > 200) {
          description += "...";
        }
      } else {
