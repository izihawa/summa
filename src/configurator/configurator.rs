...
impl Configurator {
    pub fn new(application_config_filepath: &Path) -> SummaResult<Configurator> {
        let application_config = ApplicationConfigHolder::from_file(application_config_filepath, None)?;

        std::fs::create_dir_all(&application_config.data_path).map_err(|e| Error::IOError((e, Some(application_config.data_path.clone()))))?;
        let runtime_config = RuntimeConfigHolder::from_file(&application_config.data_path.join("runtime_config.yaml"), None)?;
        runtime_config.save()?;
