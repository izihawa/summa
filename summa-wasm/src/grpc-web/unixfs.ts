...
class Data$Type extends MessageType<Data> {
    constructor() {
        super("unixfs.Data", [
            ...
            { no: 3, name: "filesize", kind: "scalar", opt: true, T: 4 /*ScalarType.UINT64*/ },
            { no: 4, name: "blocksizes", kind: "scalar", repeat: 1 /*RepeatType.PACKED*/, T: 4 /*ScalarType.UINT64*/ },
            { no: 5, name: "hashType", kind: "scalar", opt: true, T: 4 /*ScalarType.UINT64*/ },
            { no: 6, name: "fanout", kind: "scalar", opt: true, T: 4 /*ScalarType.UINT64*/ }
        ]);
    }
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* optional uint64 filesize */ 3:
                    message.filesize = reader.uint64().toString();
                    break;
                case /* repeated uint64 blocksizes */ 4:
                    if (wireType === WireType.LengthDelimited)
                        for (let e = reader.int32() + reader.pos; reader.pos < e;)
                            message.blocksizes.push(reader.uint64().toString());
                    else
                        message.blocksizes.push(reader.uint64().toString());
                    break;
                case /* optional uint64 hashType */ 5:
                    message.hashType = reader.uint64().toString();
                    break;
                case /* optional uint64 fanout */ 6:
                    message.fanout = reader.uint64().toString();
                    break;
                default:
