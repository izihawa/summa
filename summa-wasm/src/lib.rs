...
impl SummaIndexWrapper {
    ...
    pub fn new(
        ...
        default_fields: &str,
        multi_fields: &str,
    ) -> Result<SummaIndexWrapper, JsValue> {
    ...
    fn search_inner(&self, external_index_alias: &str, query: proto::Query, collectors: Vec<proto::Collector>) -> SummaResult<JsValue> {
        ...
        Ok(collector_outputs.serialize(&self.serializer).unwrap())
    }
}
