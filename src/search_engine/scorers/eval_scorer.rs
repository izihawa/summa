...
lazy_static! {
    static ref RESERVED_WORDS: HashSet<&'static str> = HashSet::from(["fastsigm", "now", "original_score", "point_fade"]);
}
...
impl EvalScorer {
    pub fn new(eval_expr: &str, schema: &Schema) -> SummaResult<EvalScorer> {
        ...
        for var_name in parsed.var_names(&slab).iter().filter(|var_name| !RESERVED_WORDS.contains((*var_name).as_str())) {
            schema.get_field(&var_name).ok_or(Error::FieldDoesNotExistError(var_name.to_owned()))?;
