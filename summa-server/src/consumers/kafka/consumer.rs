...
impl Consumer {
    #[instrument]
    pub fn new(consumer_name: &str, config: &ConsumerConfig) -> SummaServerResult<Consumer> {
    ...
    pub async fn start<TProcessor>(&self, processor: TProcessor) -> SummaServerResult<()>
    where
    ...
    pub async fn commit_offsets(&self) -> SummaServerResult<()> {
        for thread_controller in self.consumer_threads.iter() {
