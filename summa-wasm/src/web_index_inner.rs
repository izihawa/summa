...
use crate::network_directory::NetworkDirectory;
...
use summa_core::{build_fruit_extractor, default_tokenizers, FruitExtractor, QueryParser};
...
impl WebIndexInner {
    ...
    pub fn new_internal(
        ...
        let files: HashMap<String, usize> = serde_wasm_bindgen::from_value(files)?;

        let network_directory = Arc::new(NetworkDirectory::new(files, request_generator));
        let chunking_directory = ChunkedCachingDirectory::new_with_capacity_in_bytes(network_directory, 16 * 1024, 256 * 1024 * 1024, CACHE_METRICS.clone());
        crate::report_to_callback(status_callback, "status", "retrieving hotcache.bin...")?;
        let index: Index = match chunking_directory.atomic_read("hotcache.bin".as_ref()) {
            Ok(hotcache_bytes) => {
                let hotcache_directory = HotDirectory::open(chunking_directory, OwnedBytes::new(hotcache_bytes.to_vec()))?;
                Index::open(hotcache_directory)?
            }
            Err(_) => {
                crate::report_to_callback(status_callback, "status", "retrieving index...")?;
                Index::open(chunking_directory)?
            }
        };
        ...
        crate::report_to_callback(status_callback, "status", "retrieving meta.json...")?;
