...
class TreeVisitor:
    ...
    def __init__(self, ignore_nodes: Optional[Tuple[Type]] = None):
...
class TreeTransformer(TreeVisitor):
    ...
    def visit(self, node, context, parents=None):
        ...
        """
        if node.final or isinstance(node, self.ignore_nodes):
            return node
        parents = parents or []
        method = self._get_method(node)
        new_node = method(node, context=context, parents=parents)
        if parents:
            self.replace_node(node, new_node, parents[-1])
        node = new_node
        if node is not None and not node.final:
            for child in node.children:
