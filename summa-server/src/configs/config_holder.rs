...
pub trait Persistable {
    fn save(&self) -> SummaServerResult<&Self>;
}

pub trait Loadable {
    fn from_file(config_filepath: &Path, env_prefix: Option<&str>) -> SummaServerResult<Self>
    where
        Self: Sized;
}
...
impl<'a, TConfig: Serialize + Deserialize<'a>> Loadable for ConfigHolder<TConfig> {
    fn from_file(config_filepath: &Path, env_prefix: Option<&str>) -> SummaServerResult<ConfigHolder<TConfig>> {
